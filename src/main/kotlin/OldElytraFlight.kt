...
import com.lambda.client.module.modules.movement.InventoryMove
...
object OldElytraFlight : PluginModule(
    name = "OldElytraFlight",
    description = "Elytra flight for 2b2t.org.ru\"",
    category = Category.MOVEMENT,
    alias = arrayOf("EF", "OEF"),
    modulePriority = 10,
    pluginMain = OldElytraFlightPlugin
) {
    ...
    /* Acceleration */
    private val accelerateStartSpeed by setting("Start Speed", 25, 0..100, 5, { mode.value != ElytraFlightMode.BOOST && page == Page.GENERIC_SETTINGS })
    ...
    private val forwardPitch by setting("Forward Pitch", -15, -90..90, 5, { spoofPitch && mode.value != ElytraFlightMode.BOOST && page == Page.GENERIC_SETTINGS })
    ...
    private val speedControl by setting("Speed C", 1.8f, 0.0f..10.0f, 0.1f, { mode.value == ElytraFlightMode.CONTROL && page == Page.MODE_SETTINGS })
    ...
    init {
        safeListener<PacketEvent.Receive> {
            ...
            if (it.packet is SPacketEntityMetadata && isPacketFlying) {
                val packet = it.packet as SPacketEntityMetadata
    ...
    private fun SafeClientEvent.landing(event: PlayerTravelEvent) {
        when {
            ...
            LagNotifier.isPaused && LagNotifier.pauseTakeoff -> {
                holdPlayer(event)
            }
    ...
    private fun SafeClientEvent.takeoff(event: PlayerTravelEvent) {
        ...
        if (!easyTakeOff || (LagNotifier.isPaused && LagNotifier.pauseTakeoff) || player.onGround) {
            if (LagNotifier.isPaused && LagNotifier.pauseTakeoff && player.posY - world.getGroundPos(player).y > 4.0f) holdPlayer(event) /* Holds player in the air if server is lagging and the distance is enough for taking fall damage */
